...
if (typeof window === 'undefined') {
   ...
   /** Cumulative amount of time that hasn't been simulated */
   let frameDelta = 0

   // The timestamp in milliseconds of the last time the main loop was run.
   // Used to compute the time elapsed between frames.

   /** Last time the main loop was run */
   ...
   /** Weight given to more recent seconds in FPS calculation */
   let fpsAlpha = 0.9
   ...
   /** Minimum duration between FPS updates in milliseconds */
   ...
   /** Number of update steps in current frame */
   ...
   /** Whether MainLoop.start() has been called */
   let started = false
   ...
   /** Whether too many updates have been attempted */
   let panic = false
    ...
    */
   const requestAnimationFrame: (callback: (timestamp: number) => void) => number = window.requestAnimationFrame ||
      function () {
   ...
   mainLoop = {
      ...
      getSimulationTimestep: function (): number {
      ...
      setSimulationTimestep: function (timestep: number): MainLoop {
      ...
      getMaxAllowedFPS: function (): number {
      ...
      setMaxAllowedFPS: function (fps: number): MainLoop {
      ...
      resetFrameDelta: function (): number {
      ...
      setBegin: function (fun: (timestamp: number, delta: number) => void): MainLoop {
         begin = fun || begin
         return this
      },
      ...
      setUpdate: function (fun: (delta: number) => void): MainLoop {
         update = fun || update
         return this
      },
      ...
      setDraw: function (fun: (interpolationPercentage: number) => void): MainLoop {
         draw = fun || draw
         return this
      },
      ...
      setEnd: function (fun: (fps: number, panic: boolean) => void): MainLoop {
         end = fun || end
         return this
      },
